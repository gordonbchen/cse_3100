...
int main() {
	...
	for(n=1; n<=1000000; n++) {
		...
		int count = 0;
        for (i = 1; i <= 100; ++i) {  // i is at most the cube root of n, in this case, 100.
            for (j = i; j <= 100; ++j) {  // start j from i to not count duplicates.
                if (i*i*i + j*j*j == n) {
                    ++count;
                    break;  // There cannot be more than 1 j for each i.
                }
            }
            
            if (count >= 2) {  // We have found our taxi cab number.
                found = 1;
                break;
            }
        }
